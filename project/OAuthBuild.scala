...
object OAuthBuild {
  ...
  private def decodeJsonString(value: String): String =
    jsonStringEscape.replaceAllIn(value, matched => matched.matched match {
      case "\\\"" => "\""
      case "\\\\" => "\\"
      case "\\/"  => "/"
      case "\\b"  => "\b"
  ...
  def configEnv(file: File): Map[String, String] = {
    ...
    def requiredJsonString(field: String): String = {
      ...
      pattern.findFirstMatchIn(json).map(m => decodeJsonString(m.group(1))).map(_.trim).filter(_.nonEmpty)
