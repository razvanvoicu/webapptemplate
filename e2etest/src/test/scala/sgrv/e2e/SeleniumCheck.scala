...
/** Verifies Chrome + a matching driver are actually launchable before the E2E suite bothers starting the backend
  ...
  * installed; this just exercises that resolution and a real launch/quit up front, so a missing/broken Chrome
  * install fails fast with a clear message instead of surfacing as a confusing failure inside the first test.
  */
